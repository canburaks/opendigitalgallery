...
export const SegmentedControl = (props: Props) => {
  ...
  const selectedData = useMemo(
    () => props.data.find((item: DataItem) => item.value === value),
    [value]
  );
  ...
  // const cls = useMemo(() => `flex flex-row flex-nowrap h-12 p-1 shadow-inner`,[props.data]);
  ...
  return (
    <motion.div
      className={'flex flex-row flex-nowrap h-12 p-1 shadow-inner'}
      ref={observe}
    ...
    >
      {props.data.map((item: DataItem, i: number) => (
        <motion.div
          ...
          layout
        >
          <motion.label className="absolute left-0 right-0 bottom-0 top-0 text-center flex flex-col justify-center opacity-6 text-sm break-words">
            {item.label}
          </motion.label>
