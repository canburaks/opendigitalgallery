  ...
  <Tab.Group as="div" className="flex flex-col-reverse min-h-full">
    ...
    <Tab.Panels className="aspect-w-1 aspect-h-1 w-full">
      <AnimatePresence>
        {images.map((image, ix: number) => (
          <Tab.Panel
            key={`${image.product_image_id}--thumb-${ix}`}
            className="min-h-full flex justify-center items-center"
            style={{ width: PRODUCT_MAIN_IMAGE_WIDTH }}
          >
            <TabImage src={image.url} alt={image.alt || PRODUCT_IMAGE_ALT} />
            {/* <ImageWithFallback
...
const TabImage = ({ src, alt }: { src: string; alt?: string }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      style={{ width: PRODUCT_MAIN_IMAGE_WIDTH, height: '100%' }}
