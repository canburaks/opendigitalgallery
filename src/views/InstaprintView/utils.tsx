...
import {
  INSTAPRINT_FRAME_OPTIONS_MAP,
  INSTAPRINT_PRODUCT_OPTIONS_MAP,
...
export function useGenerateCartProductsFromInstaCart(): CartProduct[] {
  ...
  const cartProducts = useMemo(() => {
    ...
    instaprintCart.forEach((instaCartItem: InstaprintProduct) => {
      console.log('instaCartItem', instaCartItem);
      if (instaCartItem.priceId === -1) return;

      const instaprintPrice = getProductPrice(instaCartItem!, instaprintProduct);
      console.log('instaprintPrice', instaprintPrice);
      const currentFrame = getFrameProductFromInstaCartFrameLabel(
        instaCartItem.instaprint!.frame!,
        frames
      );
...
export function getPriceTextFromPrices(
  instaprintPrice: Price | undefined,
...
export function getProductPrice(
  instaCartItem: InstaprintProduct,
  instaprintProduct: Partial<ProductDetails>
): Price | undefined {
...
export function getFramePrice(
  instaCartItem: InstaprintProduct,
  frameProduct: Partial<ProductDetails> | undefined
): Price | undefined {
  ...
  return (
    frameProduct?.prices?.find((p: Price) => p.product_option_id === productOptionId) || undefined
  );
}
...
export function getFrameProductOptionId(instaCartItem: InstaprintProduct): number {
  let productOptionId = 0;
  const hasMat = instaCartItem?.instaprint?.mat == 'true' || instaCartItem?.instaprint?.mat == true;
...
export function getFrameProductFromInstaCartFrameLabel(
  currentProductFrameLabel: string,
  frameProducts: Partial<ProductDetails>[]
): Partial<ProductDetails> | undefined {
  const mappedFrameProduct =
    FRAME_PRODUCT_MAP[currentProductFrameLabel as keyof typeof FRAME_PRODUCT_MAP];
