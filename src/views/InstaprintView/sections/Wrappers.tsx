...
function getFrameBackgroundImage(frame: InstaprintFrameOptionsEnum): string | null {
  switch (frame) {
    case InstaprintFrameOptionsEnum.BLACK:
      return BLACK;
    case InstaprintFrameOptionsEnum.DARK_BROWN:
      return DARK_BROWN;
    case InstaprintFrameOptionsEnum.NATURAL:
      return NATURAL;
    default:
      return null;
  }
...
export const MediaWrapper = (props: Props) => {
  ...
  const hasFrame = useMemo(
    () => props.frame !== InstaprintFrameOptionsEnum.NO_FRAME,
    [props.frame]
  );
      ...
      <FrameLayer frame={props.frame}>
        <MatLayer mat={props.mat} hasFrame={hasFrame}>
          <ImageComponent
            ...
            isDefault={
              Boolean(props.mat) === false && props.frame === InstaprintFrameOptionsEnum.NO_FRAME
            }
          />
        </MatLayer>
      </FrameLayer>
...
const matVariants = {
  ...
  hasMatWithFrame: {
    width: PRINT_WIDTH_CM + 2 * MAT_THICKNESS,
    boxShadow:
      'inset 0px 2px 16px 1px rgba(0, 0, 0, 0.3), inset 0px -2px 16px 1px rgba(0, 0, 0, 0.3)',
    // height: PRINT_HEIGHT_CM2 + 2 * MAT_THICKNESS,
    padding: `${MAT_THICKNESS}px`,
  },
  noMat: {
